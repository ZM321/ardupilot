...
const AP_Param::GroupInfo AP_Beacon::var_info[] = {
    ...
    // @Values: 0:None,1:Pozyx,2:Marvelmind,3:YCHIOT
    // @User: Advanced
...
bool AP_Beacon::get_origin(Location &origin_loc) const
{
    ...
    if (is_zero(origin_lat * 1.0e-7) && is_zero(origin_lon * 1.0e-7) && is_zero(origin_alt)) {
        return false;
    }

    // return origin
    origin_loc.lat = origin_lat;
    origin_loc.lng = origin_lon;
